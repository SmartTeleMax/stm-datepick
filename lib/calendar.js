...
var locales = {
    en: {
        ...
        //    "MMMM"
        //],
        help: ["Specify the desired date. After filling the fields, press Enter. Examples" + " of how you can set the date.", 'Examples', '26 January 2015', '26.01.2015',
        //'March-April',
        'January 2015', 'yesterday'],
        //rangeMatchers: [
        //    /from\s*([\d\w.-]+)\s*to\s*([\d\w.-]+)/,
        //    /([\d\w.-]+)\s*(?:[-—])\s*([\d\w.-]+)/
        //],
        rangeHelp: ["Specify the desired date. After filling the fields, press Enter. Examples" + " of how you can set the date and time periods.", 'Examples', '26 January 2015', '26.01.2015', '26.01.2015 - 12.02.2015', 'January-February', 'January 2015', 'yesterday'],
        errors: {
    ...
    ru: {
        ...
        help: ['Укажите интересующую вас дату. После заполнения поля нажмите клавишу "Enter".' + ' Несколько примеров того, как можно задать дату, приведены ниже.', 'Примеры', '26 января 2015', '26.01.2015', 'Январь 2015' //,
        //'До 26.01.2015'
        ],
        //rangeMatchers: [
        //    /с\s*([\d\w.-]+)\s*по\s*([\d\w.-]+)/,
        //    /([\dа-я.-]+)\s*(?:[-—])\s*([\dа-я.-]+)/i
        //],
        rangeHelp: ['Укажите интересующую вас дату. После заполнения поля нажмите клавишу "Enter".' + ' Несколько примеров того, как можно задать дату и период времени, приведены ниже.', 'Примеры', '26 января 2015', '26.01.2015', '26.01.2015 - 12.02.2015', 'Январь-Март', 'Январь 2015', 'вчера'],
        errors: {
...
var Calendar = exports.Calendar = function () {
    function Calendar(container, locale, min, max, options) {
        ...
        this.min = (0, _date_range_parser.unifiedDate)(min);
        this.max = (0, _date_range_parser.unifiedDate)(max);
        ...
        this.renderedRange = new _date_range.DateRange();
        this.dateRangeParser = new _date_range_parser.DateRangeParser(this.locale);
    }
    ...
    _createClass(Calendar, [{
        ...
        value: function onMouseWheel(e, delta, deltaX, deltaY) {
            ...
            if (!$(this.container).find('.calendar-wrapper-date').is('.is-active-input') && !$(this.container).find('.dropdown').is('.opened')) {
                $('.calendar-wrapper-date').toggleClass('closed', this.direction === 'down');
            }
        }
    }, {
        key: 'onMouseScroll',
        value: function onMouseScroll() {
            ...
            for (var _ in this.monthHeaderMap) {
                if (this.monthHeaderMap.hasOwnProperty(_)) {
                    ...
                    if (scrollTop - Math.abs($(header).position().top) > 0) {
                        var date = moment.utc([parseInt($(header).data('year'), 10), parseInt($(header).data('month'), 10), 1]);
        ...
        key: 'renderYearsDropdown',
        value: function renderYearsDropdown() {
        ...
        key: 'showYearDropdownList',
        value: function showYearDropdownList() {
            $(this.dropDown).addClass('opened');
            $(this.wrapperDate).addClass('is-active-dropdown');
            this.dropDownList.style.display = 'block';
        }
    }, {
        key: 'hideYearDropdownList',
        value: function hideYearDropdownList() {
        ...
        value: function onYearDropdownChange(e) {
        ...
        value: function updateDropdownValue(date) {
        ...
        key: 'onAccessibleInput',
        value: function onAccessibleInput(e, data) {
            this.dateInput.value = this.accessibleInput.value;
            this.onInput(e, data);
        }
    }, {
        key: 'onInput',
        value: function onInput(e, data) {
            ...
            if (this.lastInputValue !== value) {
                ...
                if (parsedRange && parsedRange.start) {
                    ...
                    if (this.lastParsedDate !== startDate) {
            ...
            if (this.lastParsedRange && (e.keyCode === 13 || data && data.submit)) {
                ...
                var range = new _date_range.DateRange((0, _date_range_parser.unifiedDate)(this.lastParsedRange.start), (0, _date_range_parser.unifiedDate)(this.lastParsedRange.end));
        ...
        value: function renderDay(date) {
            ...
            if (!this.lastRange.start.isSame(date, 'month') || date.isAfter(this.max) || date.isBefore(this.min)) {
        ...
        key: 'renderWeek',
        value: function renderWeek(date, targetDate) {
            var daysInWeek = 7;
            var newDate;
        ...
        value: function renderMonth(date) {
            ...
            var range = new _date_range.DateRange(start, end);
        ...
        key: 'renderWeeks',
        value: function renderWeeks(range) {
        ...
        value: function fill(date) {
            ...
            date = (0, _date_range_parser.unifiedDate)(date);
        ...
        key: 'setSelection',
        value: function setSelection(date) {
            date = (0, _date_range_parser.unifiedDate)(date);
        ...
        key: 'getDateCell',
        value: function getDateCell(date) {
            if (!date) {
                return;
            }
            date = (0, _date_range_parser.unifiedDate)(date);
        ...
        key: 'scrollToMonth',
        value: function scrollToMonth(date) {
            date = (0, _date_range_parser.unifiedDate)(date);
            var dateString = date.year() + '' + date.month();
        ...
        key: 'highLightWeekDay',
        value: function highLightWeekDay(dayIndex) {
            this.unhighLightWeekDays();
            if (dayIndex === 0) {
                dayIndex = 7;
            } // Sunday
        ...
        key: 'unhighLightWeekDays',
        value: function unhighLightWeekDays() {
        ...
        key: 'format',
        value: function format(date, text) {
            date = (0, _date_range_parser.unifiedDate)(date);
            var d = date.date(),
                D = date.day(),
                m = date.month() + 1,
