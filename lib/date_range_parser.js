...
exports.unifiedDate = unifiedDate;

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

/* jslint esversion: 6, -W097, browser: true, strict: implied, -W034 */
/* globals $:true, jQuery:true, console: true, require */

var $ = require('jquery'); // XXX
...
var DateRangeParser = exports.DateRangeParser = function () {
    function DateRangeParser(locale) {
        _classCallCheck(this, DateRangeParser);
    ...
    _createClass(DateRangeParser, [{
        key: "localize",
        value: function localize(locale) {
        ...
        key: "precArray",
        value: function precArray(d, p, offset) {
        ...
        key: "makePrecRange",
        value: function makePrecRange(dt, p, r) {
            var ret = {};
        ...
        key: "getRange",
        value: function getRange(op, term1, term2, origin) {
            if (!term1 && !term2) {
                return {};
            }
            if (op === "<" || op === "->" || op === "-") {
        ...
        key: "checkSymbol",
        value: function checkSymbol(m, symbol) {
            var re = new RegExp('(?:^|\\W)' + symbol + '+(?:$|\\W)');
            return !!m._f.match(re);
        }
    }, {
        key: "allowedSubrange",
        value: function allowedSubrange(range, options) {
        ...
        key: "processTerm",
        value: function processTerm(term, origin, options) {
            term = $.trim(term); // XXX jQuery dependency
            ...
            if (moment_.isValid()) {
                ...
                var p = 0;
                if (this.checkSymbol(moment_, 'M')) {
                    p = 1;
                } // format contains a month
                if (this.checkSymbol(moment_, 'D')) {
                    p = 2;
                } // format contains a day
        ...
        key: "parse",
        value: function parse(v, options) {
            options = options || {};
            if (options.max) {
                options.max = unifiedDate(options.max);
            }
            if (options.min) {
                options.min = unifiedDate(options.min);
            }
            ...
            var terms = ' '.split(/\s+/)[1] ? v.split(/\s*([^<>]*[^<>-])?\s*(->|<>|<)?\s*([^<>]+)?\s*/) :
        ...
        key: "parseToDate",
        value: function parseToDate(v, options) {
            ...
            var out = {};
            if (range.start) {
                out.start = new Date(range.start);
            }
            if (range.end) {
                out.end = new Date(range.end);
            }
            return out;
        }
    }]);
...
function split(str, separator, limit) {
     ...
     */
    limit = limit === undefined ? -1 >>> 0 : // Math.pow(2, 32) - 1
...
var testCases = [{ value: 'сегодня',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 6, 9), end: UTCDateEnd(2015, 6, 9) } }, { value: 'за неделю',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 6, 3), end: UTCDateEnd(2015, 6, 9) } }, { value: 'в этом месяце',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 6, 1), end: UTCDateEnd(2015, 6, 9) } }, { value: 'в этом году',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 1, 1), end: UTCDateEnd(2015, 6, 9) } }, { value: 'вчера',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 6, 8), end: UTCDateEnd(2015, 6, 8) } }, { value: 'позавчера',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 6, 7), end: UTCDateEnd(2015, 6, 7) } }, { value: 'на прошлой неделе',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 6, 1), end: UTCDateEnd(2015, 6, 7) } }, { value: 'в прошлом месяце',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 5, 1), end: UTCDateEnd(2015, 5, 31) } }, { value: 'в прошлом году',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2014, 1, 1), end: UTCDateEnd(2014, 12, 31) } }, { value: '2015',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 1, 1), end: UTCDateEnd(2015, 6, 9) } }, { value: '2014',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2014, 1, 1), end: UTCDateEnd(2014, 12, 31) } }, { value: '1999',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(1999, 12, 31), end: UTCDateEnd(1999, 12, 31) } }, { value: 'декабрь',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2014, 12, 1), end: UTCDateEnd(2014, 12, 31) } }, { value: 'июнь',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 6, 1), end: UTCDateEnd(2015, 6, 9) } }, { value: 'март',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 3, 1), end: UTCDateEnd(2015, 3, 31) } }, { value: '5 марта',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 3, 5), end: UTCDateEnd(2015, 3, 5) } }, { value: '9 июня',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 6, 9), end: UTCDateEnd(2015, 6, 9) } }, { value: '10 июня',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: {} }, { value: '9 декабря',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2014, 12, 9), end: UTCDateEnd(2014, 12, 9) } }, { value: '9 декабря',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2017, 6, 9) },
    out: { start: UTCDate(2014, 12, 9), end: UTCDateEnd(2014, 12, 9) } }, { value: '9 декабря 2010',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2010, 12, 9), end: UTCDateEnd(2010, 12, 9) } }, { value: 'март 2010',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2010, 3, 1), end: UTCDateEnd(2010, 3, 31) } }, { value: '10.10.2010',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2010, 10, 10), end: UTCDateEnd(2010, 10, 10) } }, { value: '10.10.10',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2010, 10, 10), end: UTCDateEnd(2010, 10, 10) } }, { value: 'с 10 апреля по 20 апреля',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 4, 10), end: UTCDateEnd(2015, 4, 20) } }, { value: 'с 10 по 20 апреля',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 4, 10), end: UTCDateEnd(2015, 4, 20) } }, { value: 'с 10 по 20 апреля 2014',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2014, 4, 10), end: UTCDateEnd(2014, 4, 20) } }, { value: 'Январь-Март',
    options: { now: UTCDate(2015, 6, 9), min: UTCDate(1999, 12, 31), max: UTCDate(2015, 6, 9) },
    out: { start: UTCDate(2015, 1, 1), end: UTCDateEnd(2015, 3, 31) } }];

var runTests = exports.runTests = function runTests() {
    var okTotal = 0,
        failedTotal = 0;
    ...
    console.log('Tests runned', testCases.length, ' OK: ', okTotal, 'FAILED: ', failedTotal);
};