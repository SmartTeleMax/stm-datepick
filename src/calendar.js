...
/* globals $, jQuery, console, _, DateRange, dateRangeParser, require, currentLanguage */
...
var helpTip = _('Укажите интересующую вас дату. После заполнения поля нажмите клавишу "Enter". Несколько примеров того, как можно задать дату, приведены ниже.');
var rangeHelpTip = _('Укажите интересующую вас дату. После заполнения поля нажмите клавишу "Enter". Несколько примеров того, как можно задать дату и период времени, приведены ниже.');
var errors = {
    dateInFuture: _('Неправильно указана дата.'),
    dateNotParsed: _('Неправильно указана дата.')
    //dateInFuture: _('Дата в будущем не допустима.'),
    //dateNotParsed: _('Дата не может быть распознана.')
};
var locales = {
    en: {
        monthNames: [
...
export function Calendar(container, locale, min, max, options) {
    ...
    this.min = this.unifiedDate(min);
    this.max = this.unifiedDate(max);
...
Calendar.prototype.localize = function calendarLocalizePrototype(locale) {
    this.locale = locale;
    var strings;
    if (locales[locale]) {
...
Calendar.prototype.daysInMonth = function calendarDaysInMonthPrototype(date) {
    return moment.utc(date).endOf('month').date();
};

Calendar.prototype.firstDayOfWeek = function(date) {
...
Calendar.prototype.nearTheEdge = function() {
...
Calendar.prototype.onMouseLeave = function() {
    this.unhighLightWeekDays();
};

Calendar.prototype.onMouseWheel = function(e, delta, deltaX, deltaY) {
...
Calendar.prototype.onMouseScroll = function() {
...
Calendar.prototype.click = function() {
...
Calendar.prototype.renderYearsDropdown = function() {
...
Calendar.prototype.updateDropdownValue = function(date) {
...
Calendar.prototype.renderInput = function() {
    if (this.inputRender) {
        return;
    }
    var self = this;
...
Calendar.prototype.onAccessibleInput = function(e, data) {
    this.dateInput.value = this.accessibleInput.value;
    this.onInput(e, data);
};

Calendar.prototype.onInput = function(e, data) {
    ...
    if (this.lastInputValue !== value) {
        ...
        var parsedRange = dateRangeParser.parse(value, {min: this.min, max: this.max});
    ...
    if (this.lastParsedRange && (e.keyCode === 13 || (data && data.submit))) {
        ...
        var range = new DateRange(this.unifiedDate(this.lastParsedRange.start),
                                  this.unifiedDate(this.lastParsedRange.end));
...
Calendar.prototype.renderWeek = function(date, targetDate) {
    var daysInWeek = 7;
    var newDate;
...
Calendar.prototype.empty = function() {
...
Calendar.prototype.removeMonth = function() {
    var edge;
...
Calendar.prototype.renderMonth = function calendarRenderMonthPrototype(date) {
...
Calendar.prototype.renderMonthHeader = function(date) {
    var row = document.createElement('div');
...
Calendar.prototype.renderWeeks = function(range) {
...
Calendar.prototype.fill = function calendarFillPrototype(date) {
    if (this.isFilled) {
        return;
    }
...
Calendar.prototype.setSelection = function calendarSetSelectionPrototype(date) {
    date = this.unifiedDate(date);
...
Calendar.prototype.getDateCell = function(date) {
    if (!date) {
        return;
    }
    date = this.unifiedDate(date);
...
Calendar.prototype.unifiedDate = dateRangeParser.unifiedDate;

Calendar.prototype.scrollToMonth = function(date) {
    date = this.unifiedDate(date);
    var dateString = date.year() + '' + date.month();
